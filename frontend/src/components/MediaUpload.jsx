...
export const MediaUpload = ({ media, setMedia, testId = "media" }) => {
  ...
  return (
    <div>
      <div className="label-caps mb-2">Photos / Video (optional)</div>
      ...
      {media.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2" data-testid={`${testId}-thumbs`}>
          {media.map((m, i) => (
            <div key={m.id} className="relative w-20 h-20 rounded-md overflow-hidden border border-slate-200 bg-slate-100">
...
export const MediaThumbs = ({ media }) => {
  const token = localStorage.getItem("sh_token");
